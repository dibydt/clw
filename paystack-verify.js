...
export default {
  async fetch(request, env) {
    ...
    console.log(`[verify-paystack] Method: ${request.method}, URL: ${request.url}`);
    ...
    if (request.method !== "POST") {
      return new Response(
        JSON.stringify({
          error: `Method not allowed. Expected POST, got ${request.method}`,
        }),
        { status: 405, headers }
      );
    }
    ...
    try {
      ...
      if (!reference || !tier || !TIER_DAYS[tier]) {
        return new Response(
          JSON.stringify({ error: "Missing reference or invalid tier" }),
          { status: 400, headers }
        );
      }
      ...
      // ---- 3. Update the user's subscription in Supabase ----
      ...
      // ---- 4. Send a thank-you email (optional) ----
      const paidNaira = (result.data.amount / 100).toLocaleString();
